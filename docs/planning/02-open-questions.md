...
| **Auth (Admin)** | **Proyecto SvelteKit separado** — auth por rol `admin`; la API `/api/admin/*` está implementada | `planning/02-open-questions.md` (Q20), `use-cases/admin.md` |
| **OAuth** | Google + GitHub (callback en FastAPI; config-gated por credenciales) | `planning/02-open-questions.md` (Q4) |
| **Bootstrap Admin** | Variables de entorno `ADMIN_INITIAL_USER`, `ADMIN_INITIAL_PASSWORD` → crea admin en primer arranque, `must_change_password=true` | `planning/02-open-questions.md` (Q20) |
| **Sesiones** | Duración redes sociales: access 15min, refresh 30d rolling, sliding window | `planning/02-open-questions.md` (Q3) |
| **Imágenes** | Subida a disco local (`POST /api/v1/uploads`, servidas en `/uploads`) + URL externa; storage S3-compatible queda v2 | `entities.md`, `use-cases/recipes.md` |
